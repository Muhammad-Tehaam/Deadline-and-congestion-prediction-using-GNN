...
import tempfile
import os
from read_dataset import input_fn,return_corresponding_data
from GNN_Model import model_fn
import random
...
def train_and_evaluate(train_dir, eval_dir, config, model_dir=None):
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(train_dir, repeat=True, shuffle=True,generat_data = False),
        max_steps=int(config['RUN_CONFIG']['train_steps'])
    )

    eval_spec = tf.estimator.EvalSpec(
        input_fn=lambda: input_fn(eval_dir, repeat=False, shuffle=False,generat_data = False),
        throttle_secs=int(config['RUN_CONFIG']['throttle_secs'])
    )

    tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
    eval_results = estimator.evaluate(input_fn=lambda: input_fn(eval_dir, repeat=False, shuffle=False, generat_data=False))
    # Print and access the loss information
    #print("=========================================================================================")
    #print("Evaluation Results:", eval_results)
    #loss_value = eval_results['loss']
